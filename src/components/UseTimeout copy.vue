<template>
  <details>
    ...
    <article>
      <h3>Demo of <i>refAutoReset</i></h3>
      <button @click="showAlertOnAutoReset = true">Show alert</button>
      <transition appear>
        <div v-if="showAlertOnAutoReset" class="alert">
          This is a nice notification... that will disappear!
        </div>
      </transition>
    </article>
...
import { useTimeout, useTimeoutFn, refAutoReset } from "@vueuse/core";
...
const showAlertOnAutoReset = refAutoReset(false, 3000);
</script>
<style scoped>
.alert {
  margin: 1em;
